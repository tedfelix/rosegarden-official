...
/*
    ...
    Copyright 2023-2026 the Rosegarden development team.
...
{
public:
    PreferenceString(QString group, QString key, const QString &defaultValue) :
        m_group(group),
        m_key(key),
    ...
    QString get() const
    {
        if (!m_cacheValid) {
            ...
            QSettings settings;
            settings.beginGroup(m_group);
            m_cache = settings.value(m_key, m_defaultValue).toString();
